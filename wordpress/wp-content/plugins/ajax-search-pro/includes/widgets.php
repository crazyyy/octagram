  ...
  <p>
    ...
    if (is_array($searches))
      foreach ($searches as $search) {
        echo "<option value='".$search['id']."' ".((attribute_escape($searchid)==$search['id'])?"selected='selected'":"''").">".$search['name']."</option>";
      }
    ?>
    </select>
  </p>
...
?>
  ...
  <p>
    <label for="<?php echo $this->get_field_id('searchid'); ?>">Select the search form: </label>
    <select class="widefat" name="<?php echo $this->get_field_name('searchid'); ?>"  id="<?php echo $this->get_field_id('searchid'); ?>" >
      <option value='0' <?php echo ((attribute_escape($searchid)==0)?"selected='selected'":"''"); ?>>All</option>
    <?php
    if (is_array($searches))
      foreach ($searches as $search) {
        echo "<option value='".$search['id']."' ".((attribute_escape($searchid)==$search['id'])?"selected='selected'":"''").">".$search['name']."</option>";
  ...
  <p>
    ...
    <select class="widefat" name="<?php echo $this->get_field_name('action'); ?>"  id="<?php echo $this->get_field_id('action'); ?>" >
      <option value='0' <?php echo ((attribute_escape($action)==0)?"selected='selected'":"''"); ?>>Do Nothing</option>
      <option value='1' <?php echo ((attribute_escape($action)==1)?"selected='selected'":"''"); ?>>Redirect to Default search page</option>
      <option value='2' <?php echo ((attribute_escape($action)==2)?"selected='selected'":"''"); ?>>Do an Ajax search with the target</option>
    </select>     
  </p>
  <p>
  ...
  <p>
    ...
    if (is_array($searches))
      foreach ($searches as $search) {
        echo "<option value='".$search['id']."' ".((attribute_escape($targetid)==$search['id'])?"selected='selected'":"''").">".$search['name']."</option>";
      }
    ?>
    </select>
  </p>
      ...
      (function($){
         $(".ajaxsearhcprotop<?php echo $instancenum; ?>").each(function(){
            ...
            if (action==0) {
              ...
              $('a', this).click(function(e){
                 e.preventDefault();
                 $('#ajaxsearchpro'+id).ajaxsearchpro("searchfor", ($(this).html()));
                  $('html,body').animate({
                      scrollTop: $('#ajaxsearchpro'+id).offset().top-40},
                  'slow');
              });
            } else if (action==1) {
              ;
