<?php
  ...
  if (isset($_POST['onoff'])) {
    if ($_POST['onoffval'] == 1) {
       update_option("asp_stat", 1);
    } else if ($_POST['onoffval'] == 0) {
       update_option("asp_stat", 0);
    }
  }
  $where = "";
...
<div id="wpdreams" class='wpdreams wrap'> 
      ...
      #all span {
       ...
       <legend>Statistics Options</legend>
          <form style='float:left;margin:20px;' name="settings" action="" method="POST">
            <?php if ($onoff==0) { ?>
            <input name="onoff" class='submit green' type="submit" value="Turn On Statistics" />
            <input name="onoffval" type="hidden" value="1" /> 
            <?php } else if($onoff==1) { ?>
            <input name="onoff" class='submit red' type="submit" value="Turn Off Statistics" />
            <input name="onoffval" type="hidden" value="0" />           
            <?php } ?>
          </form>
          <form style='float:left;margin:20px;' name="settings" action="" method="POST">
            <input name="clearstatistics" class='submit' type="submit" onclick='var c=confirm("Are you sure?");if (!c) event.preventDefault();' value="Clear search Statistics" />
          </form>
          <form style='float:left;margin:20px;' name="settings" action="" method="POST">
  ...
  <ul id="tabs" class='tabs'>
      <li><a tabid="1" class='current'>Statistics for: <?php echo (isset($current_search['name'])?$current_search['name']:'All'); ?></a></li>
    ...
    (function($){
      ...
      <?php
        ...
        foreach ($top20 as $item) {
          $items1.= "['".mysql_real_escape_string($item['keyword'])."', ".$item['num']."],";
          rtrim($items1, ",");
        }
        $items2 = "";
        foreach ($last20 as $item) {
          $items2.= "['".mysql_real_escape_string($item['keyword'])."', ".$item['num']."],";
          rtrim($items2, ",");
        }
      ?>
